...
- [x] done    v0.9.2 b0097 — WebSocket real-time push: `/ws` broadcaster nu hver ny Modbus-bus-transaktion (læs/skriv via REST/CLI — cache-hits udløser intet, da de ikke rammer bussen) til alle tilsluttede klienter, i stedet for kun at være en echo-stub. Implementeret via en broadcast-callback fra `modbus_log_add()` (lagdelingen bevaret — modbus-laget kender intet til HTTP/WS) og `httpd_queue_work()` for sikker afsendelse fra vilkårlig task-kontekst. Modbus Log-fanen i /mgmt forbruger nu WS live (poll bevaret som fallback/gap-filler, sat ned til 5s). `hcfg.max_open_sockets` hævet 7→10 for at give plads til WS + samtidige GUI-faner.
